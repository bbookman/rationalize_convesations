...
class MarkdownParser:
    ...
    def extract_sections(self, file_path):
        self.logger.log("info"), ("Extracting sections from: {}".format(file_path))
        ...
        for header in high_level_headers:
            ...
            self.logger.log("info"),("Processing section: {}".format(section_title))
            ...
            while next_sibling and next_sibling.name != 'h1':
                if next_sibling.name == 'h2':
                    ...
                    self.logger.log("info"),("Processing secondary summary: {}".format(secondary_title))
                    secondary_content = []
            ...
            self.logger.log("info"),("Completed processing section: {}".format(sections[section_title]))
        self.logger.log("info"),("Sections extracted: {}".format(sections))
        
        return sections

    def parse_markdown_files(self, directory):
        self.logger.log("info"),("Parsing markdown files in directory: {}".format(directory))
        """Processes all markdown files in a directory."""
        parsed_data = {}
        for filename in os.listdir(directory):
            
            if filename.endswith(".md"):
                self.logger.log("info"),("Processing file: {}".format(filename))
                file_path = os.path.join(directory, filename)

                parsed_data[filename] = self.extract_sections(file_path)
        self.logger.log("info"),("Parsed data: {}".format(parsed_data))        
        return parsed_data
    def get_parsed_data(self):
        self.logger.log("info"),("Getting parsed data")
